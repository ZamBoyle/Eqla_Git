...
A l'image de Marty, nous allons retourner dans le passé de notre dépôt git. Celui-ci peut aussi être vu comme une ligne temporelle où se sont déroulés évênements enregistrés dans nos différents commits. Mais à l'inverse de Marty, nous ne modifierons pas le passé (pour le moment): nous serons observateurs.

Pour cela, notre convecteur temporel utilisera la commande git checkout et le nom du SHA-1 du commit, ou le nom du tag du commit ou encore le nom de la branche. Nous verrons plus tard ce qu'est une branche.
