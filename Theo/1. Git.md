...
- [genius.java](http://zamboyle.github.io/Cours/2022/Git/Files/genius.java)
...
1. git status  (qu'est-ce qui a changé ?) On voit qu'il est indiqué untracked files. Il y est indiqué nos fichiers genius.java, inutile.txt
2. git add genius.java (on l'ajoute dans la zone d'index)
...
5. Pour le fichier inutile.txt faites un git add .  Le point ajoutera tous ce qui a été détecté par git comme nouveaux, modifiés, supprimés.
...
1. git commit -m "Ajout du fichier genius.java"
...
```md
Exemple: [Ce lien pointe vers un fichier de votre dépôt](./toto.txt)
```
